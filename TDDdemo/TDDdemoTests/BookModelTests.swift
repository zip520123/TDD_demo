...
class BookModelTests: XCTestCase {
    func test_decodeBookList() {
        
        let books = BookDecoder.decodeBook(data: data)
        XCTAssertEqual(books[0].name, "The Great Gatsby")
        XCTAssertEqual(books[0].price, 599)
        XCTAssertEqual(books[1].name, "Anna Karenina")
        XCTAssertEqual(books[1].price, 459)
        XCTAssertEqual(books.count, 11)
    }
}
